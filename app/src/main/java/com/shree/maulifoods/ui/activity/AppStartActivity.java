                        ...
                        if (user.size() > 0) {
                            for (User list : user) {
                                ...
                                if (list.active.trim().equals("No")) {
                                    ...
                                    intent.putExtra("UID", list.user_ID);
                                    intent.putExtra("UName", list.user_Name);
                                    intent.putExtra("MobileNo", list.mobile_No);
                                    intent.putExtra("EmailID", list.eMail_ID);
                                    intent.putExtra("IMEINo", IMEINo.trim());
                                    intent.putExtra("FCMTokenID", FCMTokenID.trim());
                                }
                            }
                        } else {
                            commonUtil.getSnackbar(relativeLayout, "Device Not Registered " + IMEINo, "OK");
                        }
                    } else {
                        commonUtil.getSnackbar(relativeLayout, "Device Not Registered " + IMEINo, "OK");
                    }
